...
class TestCleanup:
    ...
    def test_cleanup(self, json_input):
        ...
        command_limitrange = (
            f"kubectl delete limitrange {limitrange_name} -n {namespace}"
        )
        ...
        assert (
            "deleted" in result_limitrange.lower()
        ), f"Failed to delete LimitRange '{limitrange_name}' in namespace '{namespace}'"
        ...
        assert (
            "deleted" in result_namespace.lower()
