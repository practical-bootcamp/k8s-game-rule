...
class TestCheck:
    ...
    def test_002_check_secret_via_client(self, json_input):
        ...
        assert b64decode(secret.data["API_KEY"]).decode('utf-8') == "ilovek8s", "Incorrect value for 'API_KEY' in Secret data."

        logging.info(f"Secret '{secret_name}' in Namespace '{namespace}' has the correct value for 'API_KEY'.")
    ...
    def test_003_check_secret_via_kubectl(self, json_input):
        ...
        assert secret["metadata"]["namespace"] == "ilovestudy", "Incorrect metadata.namespace."
        assert "API_KEY" in secret["data"], "Missing key 'API_KEY' in Secret data."
        assert b64decode(secret["data"]["API_KEY"]).decode('utf-8') == "ilovek8s", "Incorrect value for 'API_KEY' in Secret data."

        logging.info(f"Secret 'ilovek8s' in Namespace 'ilovestudy' has the correct value for 'API_KEY'.")
