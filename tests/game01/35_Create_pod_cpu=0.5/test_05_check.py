...
class TestCheck:
    ...
    def test_002_check_pod_client(self, json_input):
        ...
        assert cpu_request in ["0.5", "500m"], f"Expected requests.cpu to be '0.5' or '500m', but got '{cpu_request}'."
        assert memory_request == "1Gi", f"Expected requests.memory to be '1Gi', but got '{memory_request}'."
        assert cpu_limit == "1", f"Expected limits.cpu to be '1', but got '{cpu_limit}'."
        assert memory_limit == "2Gi", f"Expected limits.memory to be '2Gi', but got '{memory_limit}'."
