...
class TestCheck:
    def test_001_check_configmap_and_pod(self, json_input):
        ...
        logging.info("ConfigMap '%s' has the correct values for 'var6' and 'var7'.", configmap_name)
        ...
        assert env_vars["var6"].name == configmap_name, "Incorrect ConfigMap name for 'var6' in environment variable."
        assert env_vars["var6"].key == "var6", "Incorrect key for 'var6' in environment variable."
        assert env_vars["var7"].name == configmap_name, "Incorrect ConfigMap name for 'var7' in environment variable."
        assert env_vars["var7"].key == "var7", "Incorrect key for 'var7' in environment variable."
        logging.info("Pod '%s' has the correct environment variables from ConfigMap.", pod_name)
