...
class TestCheck:
    ...
    def test_002_check_pod(self, json_input):
        ...
        assert (
            env_var_dict["var6"] == "anotherone"
        ), "Incorrect ConfigMap reference for environment variable 'var6'."
        assert "var7" in env_var_dict, "Missing environment variable 'var7' in Pod."
        assert (
            env_var_dict["var7"] == "anotherone"
        ), "Incorrect ConfigMap reference for environment variable 'var7'."
        logging.info(
            f"Pod 'nginx' has the correct environment variables 'var6' and 'var7'."
        )
