...
class TestCheck:
    def test_001_check_configmap_and_pod(self, json_input):
        ...
        # 验证 ConfigMap
        try:
            logging.info("Checking ConfigMap '%s' in namespace '%s'", configmap_name, namespace)
        ...
        logging.info("ConfigMap '%s' has the correct values for 'var8' and 'var9'.", configmap_name)
        ...
        volume_mounts = {v.name: v.mount_path for v in pod.spec.containers[0].volume_mounts}
        assert volume_mounts["config-volume"] == "/etc/lala", "Incorrect mountPath for config-volume in Pod."
    ...
    def test_002_check_pod_details(self, json_input):
        ...
        # 验证卷和 ConfigMap 的挂载
        volumes = [v for v in pod_data["spec"]["volumes"] if v["name"] == "config-volume"]
        assert len(volumes) == 1, "Volume 'config-volume' not found in Pod."
        assert volumes[0]["configMap"]["name"] == "cmvolume", "ConfigMap 'cmvolume' not found in volume."

        logging.info("Pod '%s' has the correct volume with ConfigMap 'cmvolume'.", pod_name)
