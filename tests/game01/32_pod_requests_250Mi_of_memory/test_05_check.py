...
class TestCheck:
    def test_001_check_pod_client(self, json_input):
        ...
        memory_request = resources.requests["memory"] if resources and resources.requests else None
        assert memory_request == "250Mi", f"Expected memory request to be '250Mi', but got '{memory_request}'."
    ...
    def test_002_check_pod_kubectl(self, json_input):
        ...
        # 验证 Pod 中的内存请求
        resources = pod["spec"]["containers"][0]["resources"]
        memory_request = resources["requests"]["memory"] if resources and "requests" in resources else None
        assert memory_request == "250Mi", f"Expected memory request to be '250Mi', but got '{memory_request}'."
        logging.info("Pod '%s' has the correct memory request: '250Mi'.", pod_name)
