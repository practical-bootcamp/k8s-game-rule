...
class TestCheck:
    def test_001_check_pod_scheduled_on_correct_node_with_library(self, json_input):
        ...
        assert (
            labels.get("accelerator") == "nvidia-tesla-p100"
        ), f"Pod '{pod_name}' is not scheduled on a node with label 'accelerator=nvidia-tesla-p100'"
        logging.info(
            f"Pod '{pod_name}' is scheduled on a node with label 'accelerator=nvidia-tesla-p100'"
        )

    def test_002_verify_pod_scheduled_on_correct_node_with_kubectl(self, json_input):
        logging.debug(
            "Starting test_002_verify_pod_scheduled_on_correct_node_with_kubectl"
        )
