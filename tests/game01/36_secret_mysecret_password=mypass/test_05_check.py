...
class TestCheck:
    def test_001_check_secret_via_client(self, json_input):
        ...
        assert (
            b64decode(secret.data["password"]).decode("utf-8") == "mypass"
        ), "Incorrect value for 'password' in Secret data."

        logging.info(
            f"Secret '{secret_name}' in Namespace '{namespace}' has the correct value for 'password'."
        )
    ...
    def test_002_check_secret_via_kubectl(self, json_input):
        ...
        logging.info(
            f"Secret 'mysecret' in Namespace 'default' has the correct value for 'password'."
        )
